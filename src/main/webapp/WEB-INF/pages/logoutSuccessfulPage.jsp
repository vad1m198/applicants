<html>
<head>
<title>Logout</title>
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/salesforce-lightning-design-system.css">
</head>
<body class="slds-m-around--medium">
    <nav role="navigation" aria-label="Breadcrumbs">
        <ol class="slds-breadcrumb slds-list--horizontal slds-m-around--small">
            <li class="slds-breadcrumb__item slds-text-title--caps"><a href="${pageContext.request.contextPath}/login">Login</a></li>
        </ol>
    </nav>
    <div class="slds-text-heading--medium slds-m-around--small">Logout Successful!</div>

</body>
</html>